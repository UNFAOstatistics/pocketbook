...
<<P3desTOPRIGHT, eval=P3des, top_right_plot=P3des, fig.height=2.5, fig.width=3>>=
...
<<P3desLEFT, eval=P3des, left_plot=P3des, fig.height=5.9, fig.width=3.2>>=
# data
...
<<P3desRIGHT, eval=P3des ,right_plot=P3des, fig.height=5.9, fig.width=3.2>>=
...
<<P3desBOTTOM, eval=P3des, bottom_plot=P3des, fig.height=3, fig.width=6>>=
dat <- df[df$Year %in%  c(2000:2015) & df$FAOST_CODE < 5000,c("FAOST_CODE","Year","FAO_TABLE_NAME","FBS.PCS.PDES.KCD3D")]
...
cat_data <- map.df[!duplicated(map.df[c("FAOST_CODE")]),c("FAOST_CODE","FS.DA.ADESA.PCT3D")]
...
create_map_here(nCol=5, addNA=TRUE) 
