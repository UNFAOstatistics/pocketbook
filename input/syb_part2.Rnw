...
col.main2 <- colPart1[["Main"]][1]
## color for the grid
col.main2 <- colPart1[["Main"]][2]

source("./Rcode/theme_functions/theme.R")
source('./Rcode/plot_functions/plot_setup.R')

@



<<part2_example_text, text_chunk=TRUE>>=
...
<<part2_example_toprigth, top_right_chunk=TRUE>>=
# data
df <- mtcars
...
ggplot(dat, aes(x=factor(carb),y=horsepower,fill=factor(carb))) +
  ...
  scale_fill_manual(values=plot_colors(part = 2, 6)[["Sub"]])
@


<<part2_example_left, left_chunk=TRUE>>=
...
<<part2_example_right,right_chunk=TRUE>>=
...
<<part2_example_bottom,bottom_chunk=TRUE>>=
...
<<part2_example_end, end_chunk=TRUE>>=
@


<<part2_example_map,map_chunk=TRUE>>=
