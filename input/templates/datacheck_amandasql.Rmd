...
knitr::opts_chunk$set(list(echo=TRUE,
                           ...
                           warning=FALSE,
                           message=FALSE))
opts_chunk$set(fig.width = 13, fig.height = 7)
options(scipen=999)
#+ setup, include=FALSE
library(stringr)
library(DT)
...
## At the year level within object

```{r, results = "asis", fig.height = 14}
dat  %>% 
  count(reg,Part,Position,Year) %>% 
  mutate(Position = factor(Position, levels = c("TOPRIGHT",
                                               "LEFT",
                                               "RIGHT",
                                               "BOTTOM",
                                               "MAP")),
         Part = factor(Part, levels = spreads$SPREAD[1:22])) -> df 


for(pt in rev(unique(levels(df$Position)))){
# for(pt in rev(unique(levels(df$Part)))){

cat(glue("**Position: {pt} **"))

  df %>% filter(Position == pt) %>% 
    ggplot(., aes(x=reg, 
               y=Year,fill=n,label=n)) + 
  geom_tile() + geom_text(color = "white", size = 2.5) +
  labs(title= glue("Position: {pt}"),
    subtitle = "Rows per object for each region",
       fill = "number of rows") +
  theme_ipsum_rc() + 
    facet_wrap(~Part, ncol = 5, scales = "free_y") +
  scale_x_discrete(position = "top") +
    scale_fill_continuous(na.value = 'salmon') -> p
  print(p)
}
cat("\n\n")
```
