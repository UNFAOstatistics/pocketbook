...
- I see match here, but with Serbia. Most probably this is due to how the country codes from WDI are treated for ex-Jugoslavian countries. I think we would need to have good reasons to unwind that process.
...
- in book:
    ...
    - Uganda: 31,34,50
    - Ethiopia: 82,86,115
- i can see match for these countries!
...
read_csv("~/Lataukset/Emissions_Agriculture_Agriculture_total_E_All_Data_(Norm).csv") %>% 
  filter(
    `Country Code` %in% c(100,171,112,59,272,106,226,59,238),
    # `Area Code` %in% c(100,171,112,59,272,106,226,59,238), 
    # Year >= 1988, 
    # ElementCode == 5155,
    `Element Code` == 7231,
...
<div class="alert alert-dismissible alert-success">
  ...
  <h4>Win!</h4>
  <p>This indicator was re-computed as the source data had changed! Data now less sparse ie. less NA in tables</p>
</div>
...
d %>% 
  ...
  select(COUNTRY,YEAR,SEX,Numeric) %>% 
  spread(., key = SEX, value = Numeric) %>% 
  filter(YEAR >= 2010, COUNTRY %in% c("FIN","ITA","SRB")) %>% 
  gather(., key = sex, value = val, 3:4) %>% 
  spread(., key=YEAR,value = val) %>% 
  knitr::kable(digits = 1)
...
<div class="alert alert-dismissible alert-info">
  <button type="button" class="close" data-dismiss="alert">&times;</button>
  <p>Agree, but did not harmonise yet!!</p>
</div>
...
<div class="alert alert-dismissible alert-info">
  <button type="button" class="close" data-dismiss="alert">&times;</button>
  <p>Agree, but did not remove yet!</p>
</div>
