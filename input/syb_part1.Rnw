...
# }
# The up-to-date into from FIluippo
fao_prof <- read.csv("./input/data/FAOcountryProfile.csv", encoding = "UTF-8", stringsAsFactors = FALSE)
# Subset vars with unsd in their name & fao names
colnames <- grep("^UNSD", names(fao_prof), value=TRUE)
colnames <- colnames[-grep("CODE",colnames)]

unsd <- fao_prof[,c("FAOST_CODE","FAO_TABLE_NAME",colnames)]

for (var in names(unsd)[-1:-2]) {
  unsd[[var]] <- factor(unsd[[var]])
  for(level in unique(unsd[[var]])){
    level <- str_replace_all(level, " ", "_")
    unsd[paste("M49", level, sep = "_")] <- ifelse(unsd[[var]] == level, 1, 0)
  }
}
country_region <- unsd
write.csv(country_region,paste0(root.dir,"/input/data/country_region.csv"), row.names = FALSE)
# country_region <- fao_prof[c("FAOST_CODE","FAO_TABLE_NAME",
#                                                   "REU_BOOK",
#                                                   "RAP_BOOK",
#                                                   "RAF_BOOK",
#                                                   "RNE_BOOK")]
# names(country_region) <- c("FAOST_CODE","FAO_TABLE_NAME","REU","RAP","RAF","RNE")
# for (col in 3:6){
#   country_region[[col]][country_region[[col]] == "YES"] <- 1
#   country_region[[col]][country_region[[col]] == ""] <- 0
#   #country_region[[col]][is.na(country_region[[col]])] <- 0
# }
...
<<P1overTOPRIGHT, eval=P1over, top_right_plot=P1over, fig.height=2.5, fig.width=3>>=
...
<<P1overLEFT, eval=P1over, left_plot=P1over, fig.height=5.9, fig.width=3.2>>=
# data
...
<<P1overRIGHT, eval=P1over ,right_plot=P1over, fig.height=5.9, fig.width=3.2>>=
...
<<P1overBOTTOM, eval=P1over, bottom_plot=P1over, fig.height=3, fig.width=6>>=
# data
