...
if (region_to_report == "REU") dat <- dat[dat$FAOST_CODE == 5400,]
...
dat <- syb.df %>% filter(Year %in% c(2000:2014),
                         FAOST_CODE >= 5000) %>% select(FAOST_CODE,Year,SHORT_NAME,OA.TEAPT.POP.PPL.NO)
...
# remove duplicates
dat <- dat[!duplicated(dat[c("SHORT_NAME","Year")]),]
...
p <- p + scale_color_manual(values=plot_colors(part = syb_part, 6)[["Sub"]])
...
dat <- filter(syb.df, Year %in% 2013) %>% select(FAOST_CODE,SHORT_NAME,NV.AGR.TOTL.ZS,NV.IND.TOTL.ZS,NV.SRV.TETC.ZS)
...
ss_data <- dat_plot %>% filter(fill == "Agriculture") %>%   arrange(-value) %>% slice(1:6) %>% select(FAOST_CODE)
...
if (region_to_report == "GLO") {
   ...
   #¤ have to manually aggregate the World Bank data
    ...
    dat <- rbind(m49,m49world)
    dat <- merge(nomin,dat,by=c("FAOST_CODE","Year"))
    dat <- dat[dat$FAOST_CODE %in% c(5000,5100,5300,5205,5400,5500),]

}

if (region_to_report == "RAF"){
  # Add region key and subset
  dat <- join(dl,region_key)
  dat <- dat[which(dat$RAF),]
  dat <- merge(dat,nomin,by=c("FAOST_CODE","SHORT_NAME","Year"),all.x=TRUE)
  names(dat)[names(dat)=="NY.GDP.MKTP.KD"] <- "constant_gdp"
}
if (region_to_report == "RAP"){
  # Add region key and subset
  dat <- join(dl,region_key)
  dat <- dat[which(dat$RAP),]
  dat <- merge(dat,nomin,by=c("FAOST_CODE","SHORT_NAME","Year"),all.x=TRUE)
  names(dat)[names(dat)=="NY.GDP.MKTP.KD"] <- "constant_gdp"
}
...
if (region_to_report == "RNE"){
  # Add region key and subset
  dat <- join(dl,region_key)
  dat <- dat[which(dat$RNE),]
  dat <- merge(dat,nomin,by=c("FAOST_CODE","SHORT_NAME","Year"),all.x=TRUE)
  names(dat)[names(dat)=="NY.GDP.MKTP.KD"] <- "constant_gdp"
}


dat_plot <- dat %>%  group_by(SHORT_NAME,Year) %>% mutate(share = NV.AGR.TOTL.KD/constant_gdp*100) %>% 
  ungroup() %>%  arrange(-share)
top6 <- head(dat_plot[dat_plot$Year == 2013,]$FAOST_CODE,6)
dat_plot <- dat_plot %>% filter(FAOST_CODE %in% top6)
...
cat_data <- map.df[!duplicated(map.df[c("FAOST_CODE")]),c("FAOST_CODE","NV.AGR.TOTL.ZS")]
