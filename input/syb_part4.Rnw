...
<<P4landTOPRIGHT, eval=P4land, top_right_minitable=P4land, fig.height=top_right_plot_height, fig.width=top_right_plot_width>>=
dat <- filter(syb.df, Year %in% 2012) %>% select(FAOST_CODE,SHORT_NAME,RL.AREA.AGR.HA.SH,RL.AREA.FOR.HA.SH,RL.AREA.OTH.HA.SH)
...
dat_plot$SHORT_NAME <- factor(dat_plot$SHORT_NAME,
                                  levels=arrange(dat_plot[dat_plot$fill == "Agricultural",],-value)$SHORT_NAME )
...
dat <- syb.df[syb.df$Year %in%  2012 & syb.df$FAOST_CODE < 5000,c("FAOST_CODE","Year","SHORT_NAME","RL.AREA.ARBL.HA.SHP")]
...
dat <- filter(syb.df, Year %in% 2012) %>% select(FAOST_CODE,SHORT_NAME,RL.AREA.ARBL.HA.SH,RL.AREA.PRMNCR.HA.SH,RL.AREA.PRMNMP.HA.SH)
...
# reorder regions by the share of agricultural land
dat_plot$SHORT_NAME <- factor(dat_plot$SHORT_NAME,
                                  levels=arrange(dat_plot[dat_plot$fill == "Arable",],-value)$SHORT_NAME )
...
dat <- filter(syb.df, Year %in% 2012, FAOST_CODE < 5000) %>% select(FAOST_CODE,SHORT_NAME,RL.AREA.ARBLPRMN.HA.SH)
