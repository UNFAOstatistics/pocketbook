...
country_data <- data.frame(colx = rep("FAO", 284))
for (col in 1:ncol(tbl)){
  ...
  names(country_data)[col+1] <- names(tbl)[col]
}
country_data$colx <- NULL

for (col in 1:ncol(country_data)){
  country_data[[col]][country_data[[col]] %in% "empty"] <- NA
}
country_data <- country_data[rowSums(is.na(country_data))!=ncol(country_data), ]

print(
  xtable(country_data, caption = "Countries included"), include.rownames = F
)
@
