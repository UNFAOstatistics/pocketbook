...
<<overview_map, eval=print_overview_map, fig.width=12, fig.height= 8 ,out.width="1.7\\columnwidth", out.extra='angle=90', fig.cap="Regions and countries in the book">>=
...
p <- p + geom_path(data = gr_rob, aes(long, lat, group = group, fill = NULL), 
              linetype = "solid", color = col.main2, alpha=.1)
# Grey for the non-data regions
p <- p + geom_polygon(fill="grey80", colour = NA)
p <- p + geom_polygon(aes(fill = value_cat), colour = NA)
p <- p + geom_polygon(fill=NA,colour = alpha("white", 1/2), size=.4, guide = FALSE)
...
<<overview_table, eval=print_overview_map, results='asis'>>=
...
print(
  xtable(overview_data, caption = "Countries included"), type = "latex", table.placement = NULL, 
  booktabs = TRUE, 
