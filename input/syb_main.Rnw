...
\definecolor{part5}{cmyk}{0,0.30,0.51,0.57}
\definecolor{part6}{cmyk}{0,0.26,0.45,0.35}
...
#                                     "#C3B091", # Khaki
#                                     "#F0DC82")) # Buff
p <- p + scale_fill_manual(values = wes_palette("Cavalcanti"))
p <- p + theme(legend.position = c(0.05,0.05), 
                          legend.justification=c(0,0),
                          legend.key.size=unit(6,'mm'),
                          legend.direction = "vertical",
...
if (region_to_report != "GLO") overview.df <- overview.df[which(overview.df[[region_to_report]]),]
...
\newpart{part7}
\rowcolors{1}{part7!10}{white}
\clearpage

<<part7, child = if (include_part7) 'syb_part7.Rnw'>>=
@
...
<<metadata, child = if (include_metadata) 'syb_metadata.Rnw'>>=
@
