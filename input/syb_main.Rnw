...
subgrouping <- function(region_to_report){
  ...
  for (var in names(df)[grep(paste0(region_to_report,"_"), names(df))]) {
        ...
        df$subgroup <- ifelse(df[[var]] & is.na(df$subgroup),var,df$subgroup) 
        df$subgroup2 <- ifelse(df[[var]] & !is.na(df$subgroup),var,df$subgroup2)
  }
  df$subgroup3 <- ifelse(df$subgroup2 != df$subgroup, paste(df$subgroup, df$subgroup2, sep="_AND_"), df$subgroup)
  df$subgroup <- NULL
  df$subgroup2 <- NULL
  names(df)[names(df)=="subgroup3"] <- "subgroup"
