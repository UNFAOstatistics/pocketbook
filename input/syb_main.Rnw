...
opts_chunk$set(list(echo=FALSE,eval=TRUE,cache=FALSE,warning=FALSE,message=FALSE,cache.path="~/fao_cache/regional15/",dev="pdf",results='asis'))

knit_hooks$set(text_chunk = function(before, options, envir) {
  ...
  if (options$text_chunk == FALSE) return()
  # Set fig size
  fig.width  =  3
  fig.height =  10
     if (!before) {
...
knit_hooks$set(top_right_chunk = function(before, options, envir) {
  
     if (options$top_right_chunk == FALSE) return()
...
knit_hooks$set(left_chunk = function(before, options, envir) {

  if (options$left_chunk == FALSE) return()
     if (before) {
...
knit_hooks$set(right_chunk = function(before, options, envir) {
  
  if (options$right_chunk == FALSE) return()
     if (before) {
...
knit_hooks$set(bottom_chunk = function(before, options, envir) {
  
  if (options$bottom_chunk == FALSE) return()
     if (before) {
...
knit_hooks$set(map_chunk = function(before, options, envir) {
  
  if (options$map_chunk == FALSE) return()
     if (before) {
       return("\\begin{chart} \n")
     } else {
...
# chunk-options for map chunks

map.fig.width  <- if (region_to_report == "RAF") 6 else 15
map.fig.height <- if (region_to_report == "RAF") 12 else 8
map.out.width  <- if (region_to_report == "RAF") "1\\columnwidth" else "2\\columnwidth" 
map.out.extra  <- if (region_to_report != "RAF") 'angle=90' else 'angle=0'
# ,fig.align= "center"
...
p <- p + geom_polygon(fill="Dim Grey",colour = NA)
p <- p + geom_polygon(aes(fill = value_cat), colour = NA)
p <- p + geom_polygon(fill=NA,colour = alpha("white", 1/2), size=.7, guide = FALSE)
...
<<country_profiles, child = if (include_country_profiles) 'country_profiles.Rnw'>>=
@
