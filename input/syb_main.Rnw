...
if (region_to_report == "RNE") map.out.height  <- ".7\\columnwidth"
...
if (region_to_report == "RAF"){
  map.df <- map.df[which(map.df$RAF),]
  map.df$value_cat[map.df$RAF_Central_Africa] <- "Central Africa" 
  map.df$value_cat[map.df$RAF_East_Africa]    <- "East Africa" 
  map.df$value_cat[map.df$RAF_North_Africa]   <- "North Africa" 
  map.df$value_cat[map.df$RAF_South_Africa]   <- "South Africa" 
  map.df$value_cat[map.df$RAF_West_Africa]    <- "West Africa" 
}
if (region_to_report == "RAP") {
  map.df <- map.df[which(map.df$RAP),]
  map.df$value_cat[map.df$RAP_Central_Asia]              <- "Central Asia"
  map.df$value_cat[map.df$RAP_Eastern_Asia]              <- "Eastern Asia"
  map.df$value_cat[map.df$RAP_Southern_Asia]             <- "Southern Asia"
  map.df$value_cat[map.df$RAP_South_Eastern_Asia]        <- "South Eastern Asia"
  map.df$value_cat[map.df$RAP_Western_Asia]              <- "Western Asia"
  map.df$value_cat[map.df$RAP_Austriala_and_New_Zealand] <- "Australia and New Zealand"
  map.df$value_cat[map.df$RAP_Melanesia]                 <- "Melanesia"
  map.df$value_cat[map.df$RAP_Micronesia]                <- "Micronesia"
  map.df$value_cat[map.df$RAP_Polynesia]                 <- "Polynesia"
  map.df$value_cat[map.df$RAP_Russian_Federation]        <- "Russian Federation"
  map.df$value_cat[map.df$RAP_France]                    <- "France"
  map.df$value_cat[map.df$RAP_United_States]             <- "United States"
}
if (region_to_report == "REU"){
  map.df <- map.df[which(map.df$REU),]
  map.df$value_cat[map.df$REU_South_Eastern_Europe]   <- "South Eastern Europe"
  map.df$value_cat[map.df$REU_Other_And_EFTA]         <- "Other and EFTA"
  map.df$value_cat[map.df$REU_Caucasus_and_Turkey]    <- "Caucasus and Turkey"
  map.df$value_cat[map.df$REU_CIS_Europe]             <- "CIS Europe"
  map.df$value_cat[map.df$REU_Central_Eastern_Europe] <- "Central Eastern Europe"
  map.df$value_cat[map.df$REU_Israel]                 <- "Israel"
  map.df$value_cat[map.df$REU_Central_Asia]           <- "Central Asia"
} 
if (region_to_report == "RNE"){
  map.df <- map.df[which(map.df$RNE),]
  map.df$value_cat[map.df$RNE_Gulf_Cooperation_Council_States_and_Yemen] <- "Gulf Cooperation Council States and Yemen"
  map.df$value_cat[map.df$RNE_Other_Near_East_countries]                 <- "Near East countries"
  map.df$value_cat[map.df$RNE_North_Africa]                              <- "North Africa"
}
if (region_to_report == "GLO"){
  map.df <- map.df[which(map.df$GLO),]
  map.df$value_cat[map.df$GLO_Europe]      <- "Europe"
  map.df$value_cat[map.df$GLO_Asia]        <- "Asia"
  map.df$value_cat[map.df$GLO_Africa]      <- "Africa"
  map.df$value_cat[map.df$GLO_Oceania]     <- "Oceania"
  map.df$value_cat[map.df$GLO_Americas]    <- "Americas"
}
...
# if (region_to_report == "RAF") namedata <- map.df[which(map.df$RAF),]
# if (region_to_report == "RAP") namedata <- map.df[which(map.df$RAP),]
# if (region_to_report == "REU") namedata <- map.df[which(map.df$REU),]
# if (region_to_report == "RNE") namedata <- map.df[which(map.df$RNE),]
# if (region_to_report == "GLO") namedata <- map.df[!is.na(map.df$SHORT_NAME),]
# if (region_to_report == "COF") namedata <- map.df[!is.na(map.df$SHORT_NAME),]
# 
namedata <- the_whole[!is.na(the_whole$SHORT_NAME),]
cnames <- stats:::aggregate.formula(cbind(long, lat) ~ SHORT_NAME, data=namedata, mean)

# namedata <- the_whole[!is.na(the_whole$ADM0_NAME),]
# cnames <- stats:::aggregate.formula(cbind(long, lat) ~ ADM0_NAME, data=namedata, mean)
...
p <- p + geom_path(data = gr_rob, aes(long, lat, group = group, fill = NULL), 
              linetype = "solid", color = col.main2, alpha=.1)
# Grey for the non-data regions
p <- p + geom_polygon(data=the_whole, fill="grey80", colour = NA)
p <- p + geom_polygon(aes(fill = value_cat), colour = NA)
p <- p + geom_polygon(fill=NA,colour = alpha("white", 1/2), size=.4, guide = FALSE)
p <- p + scale_fill_manual(values=plot_colors(part = syb_part, length(unique(map.df$value_cat)))[["Sub"]])
p <- p + geom_text(data=cnames, aes(long, lat, label = SHORT_NAME, group=SHORT_NAME), size=2, color="black")
...
if (region_to_report == "GLO") map.df <- map.df; map.df <- map.df[!is.na(map.df$value_cat),]
if (region_to_report == "COF") map.df <- map.df

if(length(unique(map.df$value_cat)) >= 6) cat(paste("\\begin{landscape}"))

df <- map.df[!duplicated(map.df[c("SHORT_NAME")]),]
df <- df[c("SHORT_NAME","value_cat")]
tbl <- spread(df, value_cat, SHORT_NAME)
...
if(length(unique(map.df$value_cat)) == 2) alignment <- "lp{1.1in}p{1.1in}"
if(length(unique(map.df$value_cat)) == 3) alignment <- "lp{.9in}p{.9in}p{.9in}"
if(length(unique(map.df$value_cat)) == 4) alignment <- "lp{.7in}p{.7in}p{.7in}p{.7in}"
if(length(unique(map.df$value_cat)) == 5) alignment <- "lp{.5in}p{.6in}p{.5in}p{.5in}p{.5in}"
if(length(unique(map.df$value_cat)) == 6) alignment <- "lp{.6in}p{.6in}p{.6in}p{.6in}p{.6in}p{.6in}"
if(length(unique(map.df$value_cat)) == 7) alignment <- "lp{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}"
if(length(unique(map.df$value_cat)) == 8) alignment <- "lp{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}"
if(length(unique(map.df$value_cat)) == 9) alignment <- "lp{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}"
if(length(unique(map.df$value_cat)) == 10) alignment <- "lp{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}"
if(length(unique(map.df$value_cat)) == 11) alignment <- "lp{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}"
if(length(unique(map.df$value_cat)) == 12) alignment <- "lp{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}p{.5in}"
...
if(length(unique(map.df$value_cat)) >= 6) cat(paste("\\end{landscape}"))
@
