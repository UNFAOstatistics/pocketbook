...
knit_hooks$set(left_plot = function(before, options, envir) {
  ...
  if (options$left_plot == FALSE) return()
     if (before) {
      ...
      return(paste0("\\vspace{.6mm} \n","\\caption{",caption_text,"}","\\end{chart}} \n"))
    }
})
...
<<define_disclaimer, echo=FALSE, results='asis', eval=TRUE>>=
  ...
  cat("
    ...
    \\else
    \\includepdf[width=\\paperwidth,height=\\paperheight, offset=-10 -50]{./I4985E_Disclaimer_coffee.pdf}
    \\fi
  ")
    if (region_to_report != "COF"){
      cat("
...
Today more than ever, people are consuming coffee. As the middle classes of the developing world continues to grow, they are enjoying the beverage more frequently, and in a larger number of countries. On the other side of the supply chain are the producers. Coffee has proven to be an important economic driver for many smallholders, who provide most of the world’s beans. Today, annual output is almost nine million tonnes, one million tonnes more than a decade ago. The gross production value of green coffee now exceeds US\\$16 billion, and its export value reached US\\$24 billion in 2012. 

\\vspace{3mm}

Coffee growers can face headwinds due to inadequate access to credit, to price volatility, or to currency or interest-rate fluctuations, all of which can exclude them from some of the gains of an expanding market. Roasters and importers also experience losses as a result, but the impact is strongest further up the chain. Climate change and its impact on weather conditions also create uncertainty and may just be one of the most important threats to coffee production.

\\vspace{3mm}

This Coffee Pocketbook, which is part of the FAO Statistical Yearbook suite of products, focuses on indicators related to coffee production and consumption, but also on other factors that are directly or indirectly related to the coffee market, including poverty and broader economic trends.  The publication contains two sections: thematic spreads with data-driven visualizations, and comprehensive country and regional profiles. 

\\vspace{3mm}

FAO is strongly committed to helping countries strengthen their national statistical systems and improve the timeliness and quality of their data. It is also committed to improving the livelihoods of people along the various stages of the coffee production chain.
...
<<overview_map, eval=include_overview, fig.width=12, fig.height= 8 ,out.width="1.7\\columnwidth", out.extra='angle=90', fig.cap="Regions and countries in the book">>=
...
<<overview_table, eval=include_overview, results='asis'>>=
