...
#    }
  ...
  # crop the grid
  if (region_to_report %in% c("RAF","REU")) {
  ...
  if (region_to_report %in% c("RAP")) { # these are values from RAF data as we want to show the outline of RAF map here
    gr_rob <- gr_rob[gr_rob$lat >= min(map.plot$lat) & gr_rob$lat <= max(map.plot$lat),]
    gr_rob <- gr_rob[gr_rob$long >= 3290114 & gr_rob$long < 18198767,]
  ...
  if (region_to_report == "RAP") p <- p + coord_cartesian(xlim=c(7290114,23198767,ylim=c(1965387,8184223)))
  # if (region_to_report == "RAP") p <- p + coord_cartesian(xlim=c(2355556,8355556,ylim=c(965387,0)))
  # if (region_to_report == "RAP") p <- p + coord_cartesian(xlim=c(5355556,8355556,ylim=c(1965387,0)))
